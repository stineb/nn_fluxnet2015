nn_fVAR_fluxnet <- function( sitename, nam_target="lue_obs_evi", use_weights=ifelse( nam_target=="lue_obs_evi", TRUE, FALSE ), use_fapar=FALSE, nrep=5, dotrain=FALSE, package="nnet", makepdf=TRUE, testprofile=FALSE, overwrite=FALSE ){
  ...
  if (overwrite||!file.exists(outfil)){

    ## input file: get NN-soilm-profile data
    if (testprofile){
      filn <- paste( "./data/profile/profile_light_", nam_target, char_wgt, char_fapar, "_nn_", sitename, ".Rdata", sep="" )
    } else {
      filn <- paste( myhome, "data/nn_fluxnet/profile/profile_light_", nam_target, char_wgt, char_fapar, "_nn_", sitename, ".Rdata", sep="" )
    }

    deleteagain <- FALSE
    if (!file.exists(filn)){
      print( "downloading profile file ..." )
      # system( paste("rsync -avz bstocker@login.cx1.hpc.ic.ac.uk:/home/bstocker/data/nn_fluxnet/profile/profile_light_", nam_target, char_wgt, char_fapar, "_nn_", sitename, ".Rdata"," /alphadata01/bstocker/data/nn_fluxnet/profile/", sep="" ) )
      # deleteagain <- TRUE
      print("done.")
    }
    ...
    if ( class(out)!="try-error" ){
      ...
      if (!is.null(data)){
        ...
        # ##------------------------------------------------
        # isoilm_data <- profile_nn_light[[ sitename ]]$best_smdata[1]
        ...
        for ( isoilm_data in do_smdata ){
          ...
          # df_stat_good <- profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$df_stat
          # diff_good_bad <- profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$diff_good_bad
          ...
          isoilm_trh <- profile_nn_light[[ sitename ]][[ isoilm_data ]][[ package ]]$best_soilm_trh[1]
          ...
          if ( dotrain ) {
            ...
            if ( length(idxs_good)>30 && (nrow(df_nona) - length(idxs_good))>30 ){
              ...
              nn_vpd_vals <- array( NA, dim=c(nrow(df_nona),nrep) )
              ...
              for (irep in 1:nrep){
                ...
                nn_vpd_vals[,irep] <- as.vector( out_nn_vpd$vals )
            ...
            if (nrep>1){
              ...
              df_nona$var_nn_vpd    <- apply( nn_vpd_vals[,], 1, FUN = mean )
              df_nona$var_nn_act    <- apply( nn_act_vals[,], 1, FUN = mean )

              df_nona$var_nn_pot_sd <- apply( nn_pot_vals[,], 1, FUN = sd )
              df_nona$var_nn_vpd_sd <- apply( nn_vpd_vals[,], 1, FUN = sd )
              ...
              df_nona$var_nn_vpd    <- nn_vpd_vals[,1]
              df_nona$var_nn_act    <- nn_act_vals[,1]

              df_nona$var_nn_pot_sd <- nn_pot_vals[,1]
              df_nona$var_nn_vpd_sd <- nn_vpd_vals[,1]
            ...
            var_nn_pot <- apply( profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet[[ paste( "smtrh_", as.character( isoilm_trh ), sep="" ) ]]$var_nn_pot, 1, FUN=mean )
            var_nn_vpd <- apply( profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$var_nn_vpd, 1, FUN=mean )
            var_nn_act <- apply( profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$var_nn_act, 1, FUN=mean )
            fvar       <- apply( profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$var_nn_act / profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet[[ paste( "smtrh_", as.character( isoilm_trh ), sep="" ) ]]$var_nn_pot, 1, FUN=mean )
            idxs_good  <- profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet[[ paste( "smtrh_", as.character( isoilm_trh ), sep="" ) ]]$idxs_good
            moist      <- rep( NA, length(var_nn_act) )
            moist[idxs_good]  <- TRUE
            moist[-idxs_good] <- FALSE

            tmp <- data.frame( 
              var_nn_pot = var_nn_pot,
              var_nn_vpd = var_nn_vpd,
              var_nn_act = var_nn_act,
              fvar       = fvar,
              year_dec   = profile_nn_light[[ sitename ]][[ isoilm_data ]]$nnet$year_dec,
              moist      = moist
            )

            if (nam_target=="gpp_obs"){
              tmp$gpp_obs <- profile_nn_light[[ sitename ]][[isoilm_data]]$nnet$gpp_obs
            } else if (nam_target=="lue_obs_evi" || nam_target=="lue_obs_fpar"){
              tmp$lue_obs <- profile_nn_light[[ sitename ]][[isoilm_data]]$nnet$lue_obs
            }

            ## expand to all days, filling with NA
            tmp <- niceify( tmp, df_nona )
            df_nona$var_nn_pot    <- tmp$var_nn_pot
            df_nona$var_nn_vpd    <- tmp$var_nn_vpd
            df_nona$var_nn_act    <- tmp$var_nn_act
            df_nona$fvar          <- tmp$fvar
            df_nona$moist         <- tmp$moist
        ...
        if (length(drop_idx)>0){
          ...
          var_nn_vpd_by_smdata   <- var_nn_vpd_by_smdata[ -drop_idx, ] 
          fvar_by_smdata         <- fvar_by_smdata[ -drop_idx, ]
          moist_by_smdata        <- moist_by_smdata[ -drop_idx, ]
          nice                   <- nice[ -drop_idx, ]
        }
        ...
        if (length(lab_smdata)>1){
          ...
          colnames(var_nn_pot_by_smdata) <- paste( "var_nn_pot_", lab_smdata, sep="" )
          colnames(var_nn_act_by_smdata) <- paste( "var_nn_act_", lab_smdata, sep="" )
          colnames(var_nn_vpd_by_smdata) <- paste( "var_nn_vpd_", lab_smdata, sep="" )
          colnames(fvar_by_smdata)       <- paste( "fvar_",       lab_smdata, sep="" )
          colnames(moist_by_smdata)      <- paste( "moist_",      lab_smdata, sep="" )

          ## add NN-LUE and fLUE, mean across calculations based on different soil moisture data
          nice$var_nn_pot <- apply( var_nn_pot_by_smdata, 1, FUN=mean, na.rm=TRUE ); nice$var_nn_pot[ is.nan(nice$var_nn_pot) ] <- NA
          nice$var_nn_vpd <- apply( var_nn_vpd_by_smdata, 1, FUN=mean, na.rm=TRUE ); nice$var_nn_vpd[ is.nan(nice$var_nn_vpd) ] <- NA
          ...
          nice$moist      <- apply( moist_by_smdata, 1, FUN=mean, na.rm=TRUE )     ; nice$moist[ is.nan(nice$moist) ]           <- NA

          ## add NN-LUE and fLUE from each soil moisture data
          nice <- cbind( nice, var_nn_act_by_smdata, var_nn_pot_by_smdata, var_nn_vpd_by_smdata, fvar_by_smdata, moist_by_smdata )

          ## take that of best-performing soilm dataset as standard
          if (is.element( "soilm_obs", lab_smdata )){
            nice$fvar_obs <- fvar_by_smdata[ , "fvar_soilm_obs" ]  
          } else {
            nice$fvar_obs <- rep(NA, nrow(nice))
          }

          var_nn_pot_median <- apply( var_nn_pot_by_smdata, 1, FUN=median, na.rm=TRUE )
          var_nn_vpd_median <- apply( var_nn_vpd_by_smdata, 1, FUN=median, na.rm=TRUE )
          ...
          var_nn_vpd_max <- apply( var_nn_vpd_by_smdata, 1, FUN=max, na.rm=TRUE ); var_nn_vpd_max[ is.infinite(var_nn_vpd_max) ] <- NA
        ...
        if (length(positive)>0){
          ...
          if (makepdf) pdf( paste( myhome, "sofun/utils_sofun/analysis_sofun/fluxnet2015/fig_nn_fluxnet2015/hist/hist_fgpp_", package, "_", sitename, "_", as.character(isoilm_trh), ".pdf", sep="" ), width=8, height=6 )
            par( xaxs="i", las=1 )
            hist( even,      
          ...
          ##------------------------------------------------
          soilm     <- unlist( ifelse( is.element("soilm_obs",names(nice)), dplyr::select( nice, soilm_obs ), dplyr::select( nice, soilm_mean ) ) )
          soilm_mod <- unlist( dplyr::select( nice, soilm_mean ) )
          soilm[ is.nan( soilm ) ] <- NA
          if (!any(!is.na(soilm))) soilm <- soilm_mod
          ...
          if (makepdf){
            plot.fil <- paste( myhome, "sofun/utils_sofun/analysis_sofun/fluxnet2015/fig_nn_fluxnet2015/modobs/modobs_nn_act", sitename, ".pdf", sep="") 
          } else {
            plot.fil <- NA
          }
          out_NNall <- analyse_modobs( 
                                      ...
                                      obs, 
                                      do.plot=TRUE, 
                                      plot.title=paste( "GPP from NN_act, all days", sitename ), 
                                      nrcol=1, 
                                      plot.fil=plot.fil 
                                      )
          ...
          if (makepdf){
            plot.fil <- paste( myhome, "sofun/utils_sofun/analysis_sofun/fluxnet2015/fig_nn_fluxnet2015/modobs/modobs_nn_pot_moistdays", sitename, ".pdf", sep="") 
          } else {
            plot.fil <- NA
          }
          out_NNgood <- analyse_modobs( 
                                        ...
                                        obs, 
                                        do.plot=TRUE, 
                                        plot.title=paste( "GPP from NN_pot, moist days", sitename ), 
                                        nrcol=1, 
                                        plot.fil=plot.fil
                                        )
          ...
          if ( nrow(droughtdays) > 0 && nrow(droughtdays)>0.05*nrow(nondrgtdays) ){
            ...
            out_NNallbad <- analyse_modobs( 
                                            ...
                                            obs, 
                                            do.plot=FALSE, 
                                            plot.title=paste( "GPP from NN_act, dry days", sitename ), 
                                            nrcol=1
                                            )
          ...
          ## * RMSE of NNgood vs. NNall during good days
          if (makepdf){
            plot.fil <- paste( myhome, "sofun/utils_sofun/analysis_sofun/fluxnet2015/fig_nn_fluxnet2015/pot_vs_act_NN/pot_vs_act_NN", sitename, ".pdf", sep="") 
          } else {
            plot.fil <- NA
          }
        ...
        # if (deleteagain){
        #   system( paste("rm /alphadata01/bstocker/data/nn_fluxnet/profile/profile_", nam_target, char_fapar, "_nn_", sitename, ".Rdata", sep="" ) )
        # }
    ...
    print( paste( "nn_fvar output file already exists for site ", sitename ) )
