...
Define path variable used throughout all scripts and functions:
```{r}
myhome <- getwd()
print(paste("root directory:", myhome))
```
...
Due to its high computational demand, the profiling function is executed in parallel for each site on a High Performance Computing cluster (Imperial College London, HPC, server *CX1*). First, create a text file containing the list of sites for which profiling is to be done:
```{r}
...
1. Profile file (`./data/profile_lue_obs_nn_<sitename>.Rdata`) is read and data corresponding to the best performing soil moisture threshold is extracted from its large nested list.
...
#### Batch quality check
Get the success code and performance statistics for all sites in one dataframe and write it to the file `./successcodes.csv`.
```{r message=FALSE, warning=FALSE}
