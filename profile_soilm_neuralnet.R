profile_soilm_neuralnet <- function( sitename, nam_target="gpp_obs", use_weights=ifelse( nam_target=="lue_obs_evi", TRUE, FALSE ), use_fapar=FALSE, outdir="./data/", varnams_swc=NA, soilm_threshold=NA, packages="nnet", overwrite_profile=TRUE, nrep=1, makepdf=TRUE ){
  ...
  source( paste( myhome, "/clean_fluxnet.R", sep="" ) )
  source( paste( myhome, "/remove_outliers.R", sep="" ) )
  source( paste( myhome, "/cleandata_nn.R", sep="" ) )

  load( paste( myhome, "/data/modobs_fluxnet2015_s11_s12_s13_with_SWC_v3.Rdata", sep="" ) )
  ...
  for ( isoilm_data in varnams_swc ){
    ...
    for (ipackage in packages){
      ...
      if (length(soilm_thrsh_avl)>0){
        ...
        ##------------------------------------------------
        if (!is.na(best_soilm_trh[1])){
          if (makepdf) pdf( paste( myhome, "/fig_nn_fluxnet2015/plot_rsq_vs_smtrh/plot_rsq_vs_smtrh_", nam_target, char_fapar, "_", isoilm_data, "_", sitename, ".pdf", sep="" ) )
        ...
        if (makepdf){ 
          ...
          plot.fil <- paste( myhome, "/fig_nn_fluxnet2015/modobs_gooddays/profile/modobs_gooddays_", nam_target, char_fapar, "_", isoilm_data, "_", sitename, ".pdf", sep="" )
        }
        if (!is.na(best_soilm_trh[1])){
          out <- analyse_modobs( 
        ...
        if (makepdf){ 
          ...
          plot.dir <- paste( myhome, "/fig_nn_fluxnet2015/modobs_alldays/profile/", sep="" )
